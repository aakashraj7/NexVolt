...
import { User, Package, Heart, LogOut, ChevronDown, Sparkles, Store } from 'lucide-react';
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="p-3 border-b border-slate-100 mb-1">
            ...
            {isMerchant ? (
              <div className="mt-2.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 text-[#0066FF] border border-blue-200 text-[10px] font-bold">
                <Store className="w-3 h-3 text-[#0066FF]" /> Verified NexVolt Merchant
              </div>
            ) : (
              <div className="mt-2.5 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-50 text-cyan-800 border border-cyan-200 text-[10px] font-bold">
                <Sparkles className="w-3 h-3 text-cyan-600" /> NexVolt VIP Shopper
              </div>
            )}
          </div>
          ...
          <div className="space-y-0.5">
            {isMerchant && (
              <Link
                ...
                className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold text-[#0066FF] bg-blue-50/70 hover:bg-blue-100/80 transition"
              >
                <Store className="w-4 h-4 text-[#0066FF]" />
                <span>Merchant Portal Hub</span>
              </Link>
            )}
            ...
            {!isMerchant && (
              <Link
                to="/merchant/sign-up"
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-[#0066FF] hover:bg-slate-50 transition"
              >
